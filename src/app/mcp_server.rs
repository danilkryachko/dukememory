...
fn build_mcp_tools() -> Value {
    ...
    if let Some(items) = tools.as_array_mut() {
        items.extend([
            ...
            json!({"name":"memory_observe","description":"Record an evidence-backed bitemporal observation or causal edge; evidence_kind=file captures a project-contained file with exact SHA-256 for later freshness checks","inputSchema":{"type":"object","properties":{"id":{"type":"string"},"target_memory_id":{"type":"string"},"kind":{"type":"string","enum":["asserted","verified","contradicted","superseded","file_changed","retrieved","outcome","causes","depends_on","blocks","enables","prevents"]},"statement":{"type":"string"},"evidence_kind":{"type":"string"},"evidence_ref":{"type":"string"},"confidence":{"type":"number","minimum":0.0,"maximum":1.0},"valid_from":{"type":"number"},"valid_to":{"type":"number"},"root":{"type":"string"},"project_root":{"type":"string"},"db":{"type":"string"}},"required":["id","kind","statement","evidence_kind","evidence_ref"]}}),
