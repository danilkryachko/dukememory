    ...
    let response = match (method, path) {
        ...
        ("GET", "/memory-quality-ci") => {
            ...
            let minimal = params
                .get("minimal")
                .is_some_and(|value| value == "1" || value == "true");
            HttpResponse::ok(json!({"ci": memory_quality_ci_report(
                &conn,
                &ctx.db,
                &ctx.root,
                since_days,
        ...
        ("GET", "/fleet-dashboard-v2") => {
            let params = parse_query(query);
            let since_days = params
                .get("since_days")
                .and_then(|value| value.parse::<i64>().ok())
                .unwrap_or(7);
            HttpResponse::ok(json!({"fleet": fleet_dashboard_v2_report(db, since_days)?}))
        }
        ("GET", "/remote-sync-apply-flow") => {
            let params = parse_query(query);
            let selected = params.get("project").map(String::as_str);
            let ctx = project_context(db, selected)?;
            let conn = open_db(&ctx.db)?;
            ...
            let since_days = params
                ...
                .and_then(|value| value.parse::<i64>().ok())
                .unwrap_or(7);
            HttpResponse::ok(json!({"flow": remote_sync_apply_flow_report(
                &conn,
                &ctx.db,
                &ctx.root,
                target.as_deref(),
        ...
        ("GET", "/memory-effectiveness-lab") => {
            ...
            let since_days = params
                ...
                .and_then(|value| value.parse::<i64>().ok())
                .unwrap_or(7);
            HttpResponse::ok(json!({"effectiveness": memory_effectiveness_lab_report(
                &conn,
                &ctx.root,
                since_days,
            )?}))
        }
        ("GET", "/auto-context-budgeter-v2") => {
            let params = parse_query(query);
            let selected = params.get("project").map(String::as_str);
            let ctx = project_context(db, selected)?;
            let conn = open_db(&ctx.db)?;
            let task = params
                ...
                .unwrap_or("project memory");
            let target = params.get("target").map(String::as_str);
            HttpResponse::ok(json!({"budgeter": auto_context_budgeter_v2_report(
                &conn,
                &ctx.root,
                task,
                target,
        ...
        ("GET", "/cross-project-learning") => {
            ...
            let query_text = params
                ...
                .map(String::as_str)
                .unwrap_or("project memory");
            HttpResponse::ok(json!({"cross_project": cross_project_learning_report(
                &ctx.db,
                &ctx.root,
                query_text,
                false,
        ...
        ("POST", "/cross-project-learning/apply") => {
            let value = parse_json_body(body)?;
            let ctx = selected_project_from_body(db, &value)?;
        ...
        ("GET", "/agent-trace") => {
            ...
            let limit = params
                ...
                .and_then(|value| value.parse::<usize>().ok())
                .unwrap_or(12);
            HttpResponse::ok(json!({"agent_trace": agent_trace_report(
                &conn,
                &ctx.root,
                since_days,
                limit,
        ...
        ("GET", "/vds-sync-hardening") => {
            ...
            let since_days = params
                ...
                .and_then(|value| value.parse::<i64>().ok())
                .unwrap_or(7);
            HttpResponse::ok(json!({"vds_hardening": vds_sync_hardening_report(
                &conn,
                &ctx.db,
                &ctx.root,
                target.as_deref(),
        ...
        ("GET", "/web-control-center-v6") => {
            ...
            let since_days = params
                ...
                .and_then(|value| value.parse::<i64>().ok())
                .unwrap_or(7);
            HttpResponse::ok(json!({"control_v6": web_control_center_v6_report(
                &conn,
                &ctx.db,
                &ctx.root,
                target.as_deref(),
        ...
        ("GET", "/web-control-center-v9") => {
            ...
            let since_days = params
                ...
                .and_then(|value| value.parse::<i64>().ok())
                .unwrap_or(7);
            HttpResponse::ok(json!({"control_v9": web_control_center_v9_report(
                &conn,
                &ctx.db,
                &ctx.root,
                target.as_deref(),
        ...
        ("GET", "/fleet-supervisor") => {
            let params = parse_query(query);
            let since_days = params
                .get("since_days")
                .and_then(|value| value.parse::<i64>().ok())
                .unwrap_or(7);
            HttpResponse::ok(json!({"fleet": fleet_supervisor_report(db, since_days, false)?}))
        }
        ("POST", "/fleet-supervisor/apply") => {
            let value = parse_json_body(body)?;
            let since_days = value.get("since_days").and_then(Value::as_i64).unwrap_or(7);
            HttpResponse::ok(json!({"fleet": fleet_supervisor_report(db, since_days, true)?}))
        }
        ("GET", "/web-control-center-v10") => {
            let params = parse_query(query);
            let selected = params.get("project").map(String::as_str);
            let ctx = project_context(db, selected)?;
            let conn = open_db(&ctx.db)?;
            ...
            let since_days = params
                ...
                .and_then(|value| value.parse::<i64>().ok())
                .unwrap_or(7);
            HttpResponse::ok(json!({"control_v10": web_control_center_v10_report(
                &conn,
                &ctx.db,
                &ctx.root,
                target.as_deref(),
