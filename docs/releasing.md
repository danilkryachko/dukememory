...
Releases are tag-driven. A tag such as `v0.36.0` must exactly match the package
...
2. Run the local gate:
   ...
   scripts/release-smoke.sh target/release/dukememory 0.36.0
   ```

3. Merge the reviewed release commit to `main` and create the signed or
   annotated tag `v0.36.0` on that commit.
